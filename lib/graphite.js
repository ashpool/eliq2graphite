...
module.exports = function(config) {
  ...
  function log(period) {
    ...
    function _log(data) {
      const f = config.hostedGraphiteKey ? config.hostedGraphiteKey + '.' + format : format;
      var m = metric.create(f + '.' + period.intervaltype, data),
    ...
    return Promise.all(period.data.map(_log)).finally(client.end);
  }
