...
module.exports = function(config) {
  ...
  function _log(data) {
    return new Promise(function(resolve, reject) {
      var m = metric.create(format + (data.intervaltype || '.snapshot'), data),
        timestamp = data.time_start ? (new Date(data.time_start).getTime() + new Date(data.time_end).getTime()) / 2 : new Date(data.createddate).getTime();
